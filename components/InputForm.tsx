...
import { 
    ...
    SparklesIcon, BoltIcon, PaintBrushIcon, FaceSmileIcon, QuestionMarkCircleIcon, ShoppingBagIcon,
    BeerIcon, CocktailIcon, CurrencyDollarIcon, ClockIcon, MapPinIcon
} from './icons';
import { getAddressForCoordinates } from '../services/geminiService';
import type { FriendInput, GroupPreferences } from '../types';

interface InputFormProps {
  onFind: (friends: FriendInput[], preferences: GroupPreferences) => void;
  isLoading: boolean;
}
...
const moodCategories = [
    ...
    { name: 'Something Different', value: 'something unique and interesting like an escape room or a local market', icon: <QuestionMarkCircleIcon /> },
];
...
export const InputForm: React.FC<InputFormProps> = ({ onFind, isLoading }) => {
  ...
  const [groupPrice, setGroupPrice] = useState(priceCategories[3].value);
  const [isGettingLocation, setIsGettingLocation] = useState<string | null>(null);
  const [localError, setLocalError] = useState<string | null>(null);

  const updateFriend = (id: string, field: 'location' | 'distance' | 'vibe', value: string) => {
  ...
  const handleUseMyLocation = async (friendId: string) => {
    if (!navigator.geolocation) {
      setLocalError('Geolocation is not supported by your browser.');
      return;
    }
    setIsGettingLocation(friendId);
    setLocalError(null);
    navigator.geolocation.getCurrentPosition(
      async (position) => {
        try {
          ...
          updateFriend(friendId, 'location', address);
        } catch (e) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (friends.every(f => f.location)) {
      onFind(friends, { price: groupPrice });
    }
  };
    ...
    <form onSubmit={handleSubmit} className="space-y-8">
      ...
      <div className="space-y-4">
        <div className="text-center">
            <h3 className="text-xl font-bold text-gray-800 tracking-tight">Step 1: Set Group Preferences</h3>
        </div>
      ...
      {/* Friends' Locations & Preferences */}
       <div className="space-y-4">
        <div className="text-center">
            <h3 className="text-xl font-bold text-gray-800 tracking-tight">Step 2: Add Friends & Their Preferences</h3>
        </div>
        {friends.map((friend, index) => (
            <div key={friend.id} className="p-4 rounded-xl border border-slate-200 bg-slate-50/50 relative">
                {friends.length > 2 && (
                 ...
                 <div className="grid grid-cols-1 md:grid-cols-3 gap-x-6 gap-y-4">
                    <div className="md:col-span-2">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <button type="button" onClick={() => handleUseMyLocation(friend.id)} disabled={!!isGettingLocation}
                          ...
                          {isGettingLocation === friend.id ? <div className="animate-spin rounded-full h-5 w-5 border-b-2 border-gray-500"></div> : <MyLocationIcon className="h-6 w-6" />}
                        </button>
                      </div>
                    </div>
                    <div>
                        <label className="block text-sm font-bold text-gray-700 mb-1">Travel Preference</label>
                 ...
                 <div className="mt-4">
                    <label className="block text-sm font-bold text-gray-700 mb-2 flex items-center">
                        <FaceSmileIcon className="h-5 w-5 mr-2 text-gray-400"/>
                        Vibe Preference
                    </label>
      ...
      <div className="pt-4">
        <button type="submit" disabled={isLoading || friends.some(f => !f.location)}
          className="w-full flex items-center justify-center rounded-xl border border-transparent bg-gradient-to-br from-purple-600 to-pink-500 py-3.5 px-4 text-base font-bold text-white shadow-lg shadow-purple-500/30 hover:shadow-xl hover:shadow-purple-500/50 focus:outline-none focus:ring-2 focus:ring-purple-500 focus:ring-offset-2 disabled:from-gray-400 disabled:to-gray-500 disabled:cursor-not-allowed disabled:shadow-none transition-all duration-300 transform hover:-translate-y-1">
